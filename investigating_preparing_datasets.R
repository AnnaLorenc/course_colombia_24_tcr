...
#Option 1: the data used on the first day GSE145926_series_matrix.txt
#
...
#########################
###### Reducing the dataset - so it includes only T cells
#########################
...
sc_tcells_only_diet_clean <-DietSeurat(
  sc_tcells_only,
  layers = c("counts","data"),
  features = sc@assays$SCT@var.features,
  assays = c("SCT","RNA"),
  dimreducs = c("SCT_umap"))
object.size(sc_tcells_only_diet)%>%format(unit='auto')
#[1] "53.6 Mb"
...
# treemap(clonotypes_to_plot,
#          index=c("raw_clonotype_id"),
#        vSize="N",
#       vColor="orig.ident",
#       type="value",
...
# what cells are these expanded clonotypes?

sc


#Option 2: COMBAT data - I am discarding this option, as it has no raw TCR data available

#combat <- LoadH5Seurat("data/COMBAT/COMBAT-CITESeq-DATA.h5seurat")
